...
public final static String VERSION = (("$LastChangedRevision: 131 $".split(":"))[1]).replace('$', ' ').trim();
    
    /** This file contains a cached version of the MDS information. */
	private String mdsCacheFileName = "MDSCache.xml"; 
    private String mdsCacheFile = mdsCacheFileName;
	private String backupMdsCacheFileName = "backup"+mdsCacheFileName;
	private String backupMdsCacheFile = backupMdsCacheFileName;
            
    private static final String ARCS_MDS_SERVER = 
        "https://mds.grid.apac.edu.au:8443/wsrf/services/DefaultIndexService";
	...
	private static Log logger = 
        LogFactory.getLog(QueryEngine.class.getName());
     ...
     * 
     * @param elToQuery   The element to query for
     * @param fileToParse The XML file to parse
     * @return A <code>NodeList</code> containing the elements selected 
     *         by the query
    ...
    public void setCacheFileLocation(String cacheFile) {
    	...
    	backupMdsCacheFile = cacheFile + File.separator + backupMdsCacheFileName;
    }
     ...
     * 
     * @param cacheFileName The name to give the cache file
        ...
        try {
            ...
            if (mdsStr == null || mdsStr.length() == 0)
            {
                try {
                    ...
                    FileInputStream fis = new FileInputStream(backupMdsCacheFile);
                ...
                try {
                    ...
                    System.out.println("Could not write job string to file" + e);
                }
            }
    ...
    /**
     * Get the text value of the first child element with the specified
    ...
    public NodeList turboMDSquery(String query)
    {
    	...
    	//checkCache(); // now called isNewCacheAvailable() which returns boolean
    	isNewCacheAvailable();
    ...
    private String masterQueryMDS(String url, String xPathqueryString)
    {
		...
		try {
			 ...
			 * a check to make sure that only the Site element that conforms to
			 * GLUE // schema 1.2 is what we are getting if
			 ...
			 * (!xpathQuery.endsWith("/ancestor::node()[local-name()='Site' and " +
			 * "namespace-uri()='http://forge.cnaf.infn.it/glueschema/Spec/V12/R2']")) {
			 * 
    ...
    public static void main(String[] args) {
    	...
    	NodeList list = engine.turboMDSquery("/descendant::node()[local-name()='Site']/descendant::node()[local-name()='Cluster']/descendant::node()[local-name()='SubCluster']/descendant::node()[local-name()='SoftwarePackage']/child::node()[local-name()='Name'][contains(translate(text(),'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ'),'BEAST')]/parent::node()/parent::node()/parent::node()/parent::node()/self::node()");
	...
	private void fireNewInfoEvent(Date dateTheInfoChanged) {
		...
		if (mdsInfoListener != null && !mdsInfoListener.isEmpty()) {
			...
			synchronized (this) {
				infoListenersTargets = (Vector<GridInformationListener>) mdsInfoListener.clone();
			}

			// walk through the listener list and
			// call the gridproxychanged method in each
			Enumeration<GridInformationListener> e = infoListenersTargets.elements();
			while (e.hasMoreElements()) {
				GridInformationListener fqan_l = (GridInformationListener) e.nextElement();
	...
	// register a listener
	synchronized public void addGridInformationListener(GridInformationListener l) {
	...
	// remove a listener
	synchronized public void removeGridInformationListener(GridInformationListener l) {
		if (mdsInfoListener == null) {
