 ...
 *  A replacement for CachedMdsInformationManager that uses SQL backend
 */
public class SqlMDSInformationManager implements InformationManager {
    ...
    public Map<String, GridResource> getAllGridResources() {
        ...
        for (GridResource resource : resources) {
            ...
            String subLoc = SubmissionLocationHelpers.createSubmissionLocationString(resource);
    ...
    private String getHostname(String contactString) {
        String hostname =
                contactString.
                substring(contactString.indexOf("https://") != 0 ? 0 : 8,
    ...
    private String[] getContactStrings(String site, String fqan) {
        ...
        for (String queue : queues) {
            String[] contacts = client.getContactStringOfQueueAtSite(site, queue);
    ...
    private String[] getContactStringsForSitesWithApplication(String[] sites, String application, String version) {
        List<String> result = new LinkedList<String>();
        for (String site : sites) {
            String[] queues;

            if (version != null){
                queues = client.getQueueNamesForCodeAtSite(site, application, version);
            }
    ...
    private String getContactString(String queue, String  hostname,String jobManager){
    ...
    public String[] getAllSubmissionLocationsForApplication(String application) {
        ...
        return getContactStringsForSitesWithApplication(sites,application,null);
    }

    // this does not make any sense... the method asks for locations, but we return applications
    ...
    public Map<String, String> getApplicationDetails(String application, String version, String subLoc) {
        Map<String, String> codeDetails = new HashMap<String, String>();

        codeDetails.put(Constants.MDS_MODULES_KEY,
                    client.getModuleNameOfCodeForSubmissionLocation(subLoc, application,
                        version));
        codeDetails.put(Constants.MDS_SERIAL_AVAIL_KEY,
                    Boolean.toString(client.isSerialAvailForCodeForSubmissionLocation(subLoc, application,
                        version)));
        codeDetails.put(Constants.MDS_PARALLEL_AVAIL_KEY,
                Boolean.toString(client.isParallelAvailForCodeForSubmissionLocation(
